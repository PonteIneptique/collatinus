...
Flexion.Flexion(QObject *parent) : QObject(parent)
 ...
 QString Flexion.entete =
    "<table border=1 cellspacing=\"0\" cellpadding=\"5\">"
 QString Flexion.lina = "<tr><td>"
 QString Flexion.linb = "</td><td>"
 QString Flexion.linc = "</td></tr>"
 QString Flexion.queue = "</table>"

''' QStringList Flexion.cas = QStringList() << "nominatif"
                                               << "vocatif"
                                               << "accusatif"
                                               << "génitif"
                                               << "datif"
 ...
 QStringList Flexion.genres = QStringList() << "masculin"
                                                  << "féminin"
                                                  << "neutre"
 QStringList Flexion.nombres = QStringList() << "singulier"
                                                   << "pluriel"

 QStringList Flexion.temps = QStringList() << "présent"
                                                 << "imparfait"
                                                 << "futur"
                                                 << "parfait"
                                                 << "plus-que-parfait"
                                                 << "futur antérieur"
'''
def entreParenth(self, e):
    return QString("(%1)").arg(e)


'''*
 * \fn QString Flexion.forme (int n, label)
 ...
 *        True, retour est précédé de la morphologie.
 '''
def forme(self, n, label):
    ...
    excl = False
    firr = _lemme.irreg(n, &excl)
    if not firr.isEmpty()) lres.append(firr:
    if not excl:
        foreach (Desinence *d, ld)
            grqd = d.grq()
            # désinence trop rare, affichée :
