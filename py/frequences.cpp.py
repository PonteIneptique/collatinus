...
'''*
 * \fn QStringList Lemmat.lemmatiseF (QString f, deb)
 * \brief Lemmatise la chaîne f, tenir compte des majuscules
 *        si deb (= début de phrase) est à True, renvoie le
 *        résultat dans une QStringList.
 '''
def lemmatiseF(self, f, deb):
    QStringList res
    ml = lemmatiseM(f, deb)
 ...
 *        chaque lemme.
 '''
def frequences(self, txt):
    # J'essaie d'échanger comptage et lemmatisation Ph.
    formes = txt.split(Ch.reEspace)
    QHash<QString, freq;  # Occurrences par formes
    QStringList res
    QString forme
    c = formes.count()
    for (i = 0; i < c; ++i)
    ...
    QHash<QString, lemOcc;  # Nombre d'occurrences par lemme
    QHash<QString, QStringList>
        lemFormUnic;  # liste de uniques = par lemme
    QHash<QString, QStringList>
    ...
    for lemme in formes:
        ...
        if _hLem.isEmpty():
            ...
            lem = Ch.atone(lemme.left(lemme.indexOf(",")))
            lem.replace("j","i")
            lem.replace("J","I")
            lem.remove("<strong>")
            lem.remove("</strong>")
            format = "%1 (%2, %3, %5)\t<span style=\"color:"
